...
class RadarProfile(BaseModel):
    ...
    defense: float = Field(0, description="防守得分（接发权得分率归一化，0-100）")
    serve: float = Field(0, description="发球得分（0-100）")
    receive: float = Field(0, description="接发得分（0-100）")
    anti_pressure: float = Field(0, description="抗压得分（0-100）")
    field: float = Field(0, description="场区得分（换边适应性，0-100）")
    consecutive_score: float = Field(0, description="平均连续得分")
    consecutive_lose: float = Field(0, description="平均连续失分")
